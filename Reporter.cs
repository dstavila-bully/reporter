...
public class Reporter : MonoBehaviour
{
    ...
    void calculateCurrentLog()
    {
        ...
        if( collapse )
        {
            for( int i = 0 ; i < collapsedLogs.Count ; i++ )
            {
                ...
                if( log.logType == _LogType.Log       && !showLog     ||
                    log.logType == _LogType.Warning   && !showWarning ||
                    log.logType == _LogType.Error     && !showError   ||
                    log.logType == _LogType.Assert    && !showError   ||
                    log.logType == _LogType.Exception && !showError )
                    continue;
        ...
        else
        {
            for( int i = 0 ; i < logs.Count ; i++ )
            {
                Log log = logs[i];
                if( 
                    log.logType == _LogType.Log       && !showLog     ||
                    log.logType == _LogType.Warning   && !showWarning ||
                    log.logType == _LogType.Error     && !showError   ||
                    log.logType == _LogType.Assert    && !showError   ||
                    log.logType == _LogType.Exception && !showError )
    ...
    IEnumerator readInfo(  )
    {
        ...
        // @NOTE(Dmitrii): Since unity 5.4 webplayer is no longer supported, may need this for webgl builds...
        // Currently just disabling it.
